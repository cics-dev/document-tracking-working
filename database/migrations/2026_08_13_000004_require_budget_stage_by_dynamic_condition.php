...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if ($conditionId && $budgetOfficeId) {
            DB::table('document_flow_stages')
                ->where('office_id', $budgetOfficeId)
                ->where('stage_type', 'routing')
                ->update([
    ...
    public function down(): void
    {
        ...
        if ($budgetOfficeId) {
            DB::table('document_flow_stages')->where('office_id', $budgetOfficeId)->where('stage_type', 'routing')->update([
                'workflow_condition_id' => null, 'condition_operator' => 'equals', 'condition_value' => null,
                'is_required' => false, 'is_selectable' => true, 'updated_at' => now(),
            ]);
        }
    }
};
