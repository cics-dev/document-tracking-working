<form wire:submit="submitDocument" class="max-w-7xl mx-auto bg-white dark:bg-slate-950 rounded-xl shadow-[0_0_10px_0_rgba(0,0,0,0.5)] dark:shadow-[0_0_10px_0_rgba(255,255,255,0.08)] overflow-hidden p-6 relative text-gray-900 dark:text-white">
    ...
    <div class="hidden" id="progress-template">
        <div class="flex items-center justify-between bg-green-50 dark:bg-slate-900/70 border border-green-200 dark:border-green-800 px-3 py-2 rounded-md mb-2 animate-glow">
            <div class="flex items-center min-w-0 flex-1">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex justify-between items-center mb-1">
                        <p class="text-xs font-medium text-green-900 dark:text-white truncate file-name max-w-[120px]">filename.pdf</p>
                        <p class="text-xs text-green-700 dark:text-white upload-mb">0 MB / 0 MB</p>
                    </div>
                    ...
                    <div class="flex justify-between items-center mt-1">
                        <p class="text-xs text-green-700 dark:text-white">
                            <span class="upload-status">Preparing...</span>
                        </p>
                        <p class="text-xs text-green-700 dark:text-white upload-speed">0 MB/s</p>
                    </div>
                </div>
            </div>
            <button type="button" class="cancel-upload text-green-700 dark:text-white hover:text-green-900 dark:hover:text-green-300 p-1 ml-2">
    ...
    <div class="hidden" id="complete-template">
        <div class="flex items-center justify-between bg-green-50 dark:bg-slate-900/70 border border-green-200 dark:border-green-800 px-3 py-2 rounded-md mb-2">
            <div class="flex items-center min-w-0 flex-1">
                <div class="mr-3 flex-shrink-0 text-green-500">
                    <flux:icon icon="check-circle" class="size-4" />
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-medium text-green-900 dark:text-white truncate file-name">filename.pdf</p>
                    <p class="text-xs text-green-700 dark:text-white">Upload complete • <span class="file-size">0 MB</span></p>
                </div>
            </div>
            <button type="button" class="remove-file text-green-700 dark:text-white hover:text-green-900 dark:hover:text-green-300 p-1 ml-2">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between border-b pb-2 mb-4">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ $revision_document_number ?: 'Document Details' }}</h2>
        ...
        @if($redirect_mode != 'revision')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <flux:field>
                        <flux:label class="dark:text-white">Document Number @if($is_manual_document_number)<span class="text-red-500">*</span>@endif</flux:label>
                        ...
                        @if($is_manual_document_number)
                            <flux:input 
                                ...
                                required
                                class:input="dark:bg-slate-950 dark:text-white dark:border-slate-700"
                            />
                        @else
                            <div class="relative">
                                <flux:input 
                                    ...
                                    disabled 
                                    class="bg-gray-100 text-gray-500 italic border-dashed dark:bg-slate-800 dark:text-white"
                                    class:input="dark:bg-slate-950 dark:disabled:bg-slate-950 dark:text-white dark:disabled:text-white/50 dark:border-slate-700 dark:disabled:border-slate-700"
                                />
                                <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                    <flux:icon icon="sparkles" class="size-4 text-gray-400 dark:text-white" />
                                </div>
                            </div>
                        @endif
    ...
    <div class="mb-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <flux:field>
                    <flux:label class="dark:text-white">Type of Document <span class="text-red-500">*</span></flux:label>
                    <flux:select class="dark:bg-slate-950 dark:text-white dark:border-slate-700" 
                        wire:model="document_type_id" 
                        placeholder="Choose document type..." 
                        wire:change="handleUpdateDocumentType"
                        class="dark:bg-slate-950 dark:text-white dark:border-slate-700"
            ...
            @if ($document_type_id)
                @if ($document_type === 'RLM')
                    <div>
                        <flux:field>
                            <flux:label class="dark:text-white">{{ $document_type === 'RLM' ? 'For' : 'To' }} <span class="text-red-500">*</span></flux:label>
                            <flux:select wire:model="document_to_id" placeholder="Choose recipient..." disabled class="dark:bg-slate-950 dark:text-white dark:border-slate-700">
                ...
                @elseif ($document_type === 'ECLR' || $document_type === 'Intra')
                    <div>
                        <flux:field>
                            <flux:label class="dark:text-white">To <span class="text-red-500">*</span></flux:label>
                            <flux:input
    ...
    <div class="mb-8 bg-gray-50/80 dark:bg-slate-900/80 p-5 rounded-lg border border-gray-100 dark:border-slate-700">
        <flux:label class="mb-2 text-gray-900 dark:text-white">CF to Offices</flux:label>

        <div class="flex items-center gap-3">
            <div class="flex-1">
                <flux:select class="dark:bg-slate-950 dark:text-white dark:border-slate-700" wire:model="selected_cf_office" placeholder="Select office...">
                    @foreach ($offices as $office)
                        <flux:select.option value="{{ $office->id }}">{{ $office->name }}</flux:select.option>
                    @endforeach
                </flux:select>
    ...
    <div class="mb-8">
        ...
        <div class="mb-4">
            <flux:input wire:model="thru" type="text" autocomplete="thru" label="Thru" label:class="dark:text-white" class:input="dark:bg-slate-950 dark:text-white dark:border-slate-700" />
        </div>
        @endif

        <div class="mb-4">
            <flux:field>
                <flux:label class="dark:text-white">Subject <span class="text-red-500">*</span></flux:label>
                <flux:input 
        ...
        <div class="space-y-2">
            <flux:label class="dark:text-white">Content <span class="text-red-500">*</span></flux:label>
            <div wire:init="loadInitialContent">
                <div wire:ignore class="border border-gray-200 rounded-lg overflow-hidden shadow-sm dark:border-slate-700">
                    <div id="quill-editor" style="min-height: 200px;" class="bg-white dark:bg-slate-950 text-gray-900 dark:text-white"></div>
    ...
    @if ($document_type != 'IOM')
        @if ($document_type == 'RLM')
        <div class="mb-8 bg-gray-50/80 dark:bg-slate-900/80 p-5 rounded-lg border border-gray-100 dark:border-slate-700">
            <flux:label class="mb-3 dark:text-white">Routing Requirements <span class="text-gray-500 dark:text-white/60 font-normal">- select applicable review offices</span></flux:label>
            <div class="h-4"></div>
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-1">
                    <flux:heading size="lg" class="dark:text-white">Signatories</flux:heading>
                ...
                @foreach ($signatories as $index => $signatory)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
                        ...
                        <div class="md:col-span-1 flex justify-end">
                            ...
                            @else
                                <div class="mt-2 text-gray-400 dark:text-white/50" title="Required Signatory">
        ...
        <div class="mb-8">
            <flux:label class="mb-2 text-gray-900 dark:text-white">Attachments <span class="text-slate-500 dark:text-white/60 text-xs font-normal">(Max 100MB per file)</span></flux:label>

            <div class="flex flex-col md:flex-row gap-4">
                <div class="w-full md:w-1/3">
                    <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 dark:bg-slate-900 dark:hover:bg-slate-800 transition-colors duration-200">
                        <div class="flex flex-col items-center justify-center p-4 text-center">
                            <flux:icon icon="cloud-arrow-up" class="size-6 text-gray-500 dark:text-white/60 mb-2" />
                            <p class="text-xs text-gray-700 dark:text-white mb-1">Click to upload or drag and drop</p>
                    ...
                    <div id="file-list" class="space-y-2">
                        @if (!empty($attachments) || !empty($existingAttachments))
                            @foreach ($existingAttachments as $file)
                                <div wire:key="attachment-{{ $file['id'] }}" class="flex items-center justify-between bg-blue-50 dark:bg-slate-950/80 px-3 py-2 rounded-md border border-blue-200 dark:border-blue-800">
                            ...
                            @foreach ($attachments as $attachment)
                                <div class="flex items-center justify-between bg-gray-50 dark:bg-slate-950 px-3 py-2 rounded-md border border-gray-200 dark:border-slate-700">
                                    <div class="flex items-center min-w-0">
                                        <div class="mr-2 flex-shrink-0 text-gray-400 dark:text-white/50">
                                            <flux:icon icon="document" class="size-4" />
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-xs font-medium text-gray-900 dark:text-white truncate">
    ...
    <div class="flex flex-col-reverse sm:flex-row justify-between items-center pt-6 border-t border-gray-200 dark:border-slate-700 gap-4">
        <flux:button 
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <flux:button 
                ...
                variant="primary"
                icon="eye" 
                class="bg-gray-300 hover:bg-gray-400 dark:bg-slate-700 dark:hover:bg-slate-600 text-gray-900 dark:text-white border-transparent w-full sm:w-auto"
            >
                Preview
            </flux:button>
