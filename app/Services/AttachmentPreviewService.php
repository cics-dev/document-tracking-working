...
class AttachmentPreviewService
{
    ...
    public function response(string $storagePath)
    {
        ...
        if ($extension === 'docx') {
            $path = $this->convertDocxToPdf($path);
    ...
    {
        $extension = strtolower(pathinfo($storagePath ?? '', PATHINFO_EXTENSION));
        if ($extension === 'pdf' || in_array($extension, self::OFFICE_EXTENSIONS, true)) {
            return 'pdf';
        }
    ...
    private function convertDocxToPdf(string $source): string
    {
        $html = $this->docxHtml($source);
    ...
    private function docxHtml(string $source): string
    {
        ...
        $dom = new DOMDocument;
        abort_unless(@$dom->loadXML($xml), 422, 'The DOCX file is invalid.');
        $xpath = new DOMXPath($dom);
        $xpath->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');
        $html = '';
        foreach ($xpath->query('//w:body/w:p | //w:body/w:tbl') as $node) {
            if ($node->localName === 'p') {
                $html .= '<p>'.e($this->nodeText($xpath, $node, './/w:t')).'</p>';
            } else {
                $html .= '<table>';
                foreach ($xpath->query('.//w:tr', $node) as $row) {
                    $html .= '<tr>';
                    foreach ($xpath->query('./w:tc', $row) as $cell) {
                        $html .= '<td>'.e($this->nodeText($xpath, $cell, './/w:t')).'</td>';
                    }
                    $html .= '</tr>';
                }
                $html .= '</table><br>';
    ...
    private function zipEntry(string $source, string $entry, bool $required = true): ?string
    {
        ...
        $zip->close();
        if ($required) {
            abort_unless($contents !== false, 422, 'The office file does not contain previewable content.');
        }
